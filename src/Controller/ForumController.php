...
//            $this->log('Module class "'.$GLOBALS['FE_MOD'][$objModule->type].'" (module "'.$objModule->type.'") does not exist', 'Ajax getFrontendModule()', TL_ERROR);
            ...
            switch($action) {
                ...
                case "send":
                    ...
                    try {
                        ...
                        $notification->send(\Contao\StringUtil::deserialize($result['mail_new_pm'],true));
                    } catch (\Throwable $e) {
    ...
    private function getMessages($sender_id, $recipient_id, $offset) {
		...
		if ($sender_id == $recipient_id) {
			$sql = "SELECT message, sender_id, dt_created, subject FROM tl_c4g_forum_pn WHERE sender_id = ? AND recipient_id = ?	ORDER BY dt_created ASC";
            $limit = 5;
            $offset_sql = 0;
		} else {
			$sql = "SELECT message, sender_id, dt_created, subject FROM tl_c4g_forum_pn WHERE (sender_id = ? OR recipient_id = ?) AND (sender_id = ? OR recipient_id = ?) ORDER BY dt_created DESC"; 
            $limit = 500;
		}

		$messages = Database::getInstance() 
        ->prepare($sql) 
        ->limit($limit, $offset_sql)
        ->execute($sender_id, $sender_id, $recipient_id, $recipient_id);
